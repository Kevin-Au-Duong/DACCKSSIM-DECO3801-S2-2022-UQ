...
public class VehicleStandardInput : MonoBehaviour
	...
	void FixedUpdate ()
		...
		float steerInput = Mathf.Clamp(CrossPlatformInputManager.GetAxis(steerAxis), -1.0f, 1.0f);
		float handbrakeInput = Mathf.Clamp01(CrossPlatformInputManager.GetAxis(handbrakeAxis));
		...
		if (throttleAndBrakeInput == ThrottleAndBrakeInput.SeparateAxes)
			{
			forwardInput = Mathf.Clamp01(CrossPlatformInputManager.GetAxis(throttleAxis));
			reverseInput = Mathf.Clamp01(CrossPlatformInputManager.GetAxis(brakeAxis));
			}
		else
			{
			forwardInput = Mathf.Clamp01(CrossPlatformInputManager.GetAxis(throttleAndBrakeAxis));
			reverseInput = Mathf.Clamp01(-CrossPlatformInputManager.GetAxis(throttleAndBrakeAxis));
			}
